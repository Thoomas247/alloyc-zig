//! Conformance suite for the Alloy compiler: every language rule exercised
//! through the public pipeline, in the interpreter ('alloyc run') and, where
//! clang is available, through native code generation ('alloyc build'). The
//! expect* helpers at the top are the whole vocabulary; a test that needs
//! extra modules supplies them through TestSources.
...
const Module = compilation_module.Module;
const compiler_version = compilation_module.compiler_version;
...
test "clean module passes all stages" {
    ...
    // 11 tokens plus the end_of_file marker
...
// the same clang lookup the driver uses, so a test and a build agree
fn testClang(arena: std.mem.Allocator) ?[]const u8 {
    return toolchain.findClang(arena, std.testing.io, null);
}
...
const TestPackage = struct {
    ...
    fn loadNoFiles(_: ?*anyopaque, _: std.mem.Allocator, _: []const u8) anyerror!?[]const u8 {
        return null;
    }
};
...
test "engine parity fixes: void bindings, cast comparisons, shifts, printf, exit codes" {
    // binding a call that yields nothing is a checker error, not a
    // codegen-only one
    try expectCheckErrors(
        \\fn nothing() {}
        \\fn main() -> i32 {
        ...
        \\    return 0;
        \\}
    , &.{"a binding needs a value, and this expression yields none"});
    // a '<' after a cast target is a comparison, not a generic list
    try expectRuns(
        \\fn main() -> i64 {
        \\    var n: i64 = 3;
        \\    var limit: i64 = 5;
        \\    if (n to i32 < limit) { return 1; }
        \\    return 0;
        \\}
    , 1, "");
    // grouped literals still make fixed arrays; a fixed array behind
    // '&var' still slices mutably
    try expectChecks(
        \\fn fill(values: &var [i32 : 3]) {
        \\    const window = &var values[0..2];
        \\    window[0] = 1;
        \\}
        \\fn main() -> i32 {
        \\    var values = [0 : (3)];
    ...
    // a shift amount must fit the operand's width in both engines
    try expectRunFault(
        \\fn main() -> i32 {
        \\    var byte: u8 = 1;
        \\    var amount: u8 = 8;
        \\    return (byte << amount) to i32;
        \\}
    , "shift amount out of range");
    // an unsigned exit code truncates instead of aborting
    try expectRuns(
        \\fn main() -> u64 {
        \\    return 18446744073709551615;
        \\}
    , -1, "");
    // printf width, precision, '.*', hex, and mixed-sign shifts agree
    // across engines
    try expectBuildsAndRuns("printf_parity",
        \\extern printf(format: &[u8], ...) -> i32;
        \\fn main() -> i32 {
        \\    var value: i32 = 1;
        \\    var amount: u32 = 4;
        \\    printf("%5d|%-4s|%.2f|%x|%X|%o|%.*s|%d\n", 42, "ab", 3.14159, 255, 255, 8, 3, "hello", value << amount);
        \\    return 0;
        \\}
    , 0, "   42|ab  |3.14|ff|FF|10|hel|16\n");
}
...
test "temporaries live to the end of the statement" {
    // a view into a temporary receiver stays valid for the statement, a
    // 'while' condition's temporaries drop per iteration, and a match
    // arm leaving early drops the materialized subject (section 5.5)
    try expectBuildsAndRuns("statement_temporaries",
        \\type Box = struct { data: *var [u8] };
        \\type Holder = enum { Empty, Data: *var [u8] };
        \\fn make(fill: u8) -> Box {
        \\    var data: *var [u8] = new [fill : 3];
        \\    return Box { .data = move data };
        \\}
        \\fn hold(fill: u8) -> Holder {
        \\    var data: *var [u8] = new [fill : 3];
        \\    return Holder::Data(move data);
        \\}
        \\fn view(self box: &Box) -> &[u8] { return &box.data; }
        \\fn first(bytes: &[u8]) -> u8 { return bytes[0]; }
        \\fn main() -> i32 {
        \\    var total: i32 = first(&make(7).view()) to i32;
        \\    var rounds: i32 = 0;
        \\    while (make(1).view().length() == 3 && rounds < 2) {
        \\        rounds += 1;
        \\    }
        \\    while (true) {
        \\        match (hold(2)) {
...
test "a type containing itself by value is an error, not a crash" {
    try expectCheckErrors(
        \\type Node = enum { Leaf, Wrap: Node };
        \\fn main() -> i32 { return 0; }
    , &.{"type 'Node' contains itself by value and has no finite size: hold the recursion through '*Node', '&Node', or a slice (section 4.9)"});
    try expectCheckErrors(
        \\type Tree = struct { value: i32, left: Tree };
        \\fn main() -> i32 { return 0; }
    , &.{"type 'Tree' contains itself by value and has no finite size: hold the recursion through '*Tree', '&Tree', or a slice (section 4.9)"});
    // recursion through a pointer or a slice is the finite form
    try expectChecks(
        \\type Node = enum { Leaf, Wrap: *Node, Many: &[Node] };
        \\type Tree = struct { value: i32, left: *Tree };
        \\fn main() -> i32 { return 0; }
    );
}
